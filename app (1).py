...
def chart_rsf_vendors(df: pd.DataFrame, amount_col: str, vendor_col: str) -> go.Figure:
    ...
    sub = df[df[vendor_col].isin(top_vendors)].copy()

    fig = go.Figure()
    # Normal transactions
    normal = sub[~sub['rsf_flag']]
    fig.add_scatter(x=normal[vendor_col], y=normal[amount_col],
                    mode='markers', name='Normal',
                    marker=dict(color='#00e5ff', size=6, opacity=0.6))
    # Flagged transactions
    flagged = sub[sub['rsf_flag']]
    fig.add_scatter(x=flagged[vendor_col], y=flagged[amount_col],
                    mode='markers', name='RSF Flagged',
                    marker=dict(color='#ff3b5c', size=9, symbol='x',
                                line=dict(width=2)))
    fig.update_layout(**PLOT_LAYOUT,
